...
let allChains = [];
let infoDiv, resetBtn, undoBtn, unitBtn, newLineBtn, exitBtn;
...
async function init() {
  ...
  // EXIT BUTTON (Replaces standard Stop AR)
  ...
  arButton.addEventListener('click', () => {
    // Optional: Hide specific elements if needed
  });

  // Show/Hide buttons based on AR session state
  // Show/Hide buttons based on AR session state
  renderer.xr.addEventListener('sessionstart', () => {
    exitBtn.style.display = 'block';
    arButton.style.display = 'none'; // Hide the default 'Stop AR' button
  });
